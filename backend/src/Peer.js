...
class Peer extends EventEmitter
{
	...
	_handlePeer()
	{
		if (this.socket)
		{
			this.socket.on('disconnect', () =>
			{
				...
				logger.log('"disconnect" event [id:%s]', this.id);
	...
	addPermission(newPermission) {
		if (!this._permissions.includes(newPermission))
		{
			this._permissions.push(newPermission);

			logger.info('addPermission() | [newPermission:"%s]"', newPermission);

			this.emit('gotPermission', { newPermission });
		}
	}
	...
	removePermission(oldPermission)
	{
		if (this._permissions.includes(oldPermission))
		{
			this._permissions = this._permissions.filter((permission) => permission !== oldPermission);

			logger.info('removePermission() | [oldPermission:"%s]"', oldPermission);

			this.emit('lostPermission', { oldPermission });
		}
	}
